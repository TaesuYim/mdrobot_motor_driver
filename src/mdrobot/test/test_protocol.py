...
A fake transport is injected (no real serial port) to verify the transmitted
frames and the response parsing.
"""
...
class FakeTransport:
    """Fake transport that serves reads sequentially from a buffer and records write/flush."""
...
def test_write_register_echo_ok():
    ...
    transport = FakeTransport(request)  # the 0x06 response echoes the request
